...
export function funcInitTable(strTable = "") {
  ...
  */
  ...
  if (strTable === modSchema.constCV_Contact) {
    aryTemp = modModel.divTableCVContact.getElementsByClassName(
      "clhCopy" + strTable + "DataRowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVContact.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_CurrentFocus) {
    aryTemp = modModel.divTableCVCurrentFocus.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVCurrentFocus.removeChild(elTemp);
      intNum = 0;
    }
  }
  if (strTable === modSchema.constCV_Education) {
    aryTemp = modModel.divTableCVEducation.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVEducation.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_Experience) {
    aryTemp = modModel.divTableCVExperience.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVExperience.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_Highlights) {
    aryTemp = modModel.divTableCVHighlights.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVHighlights.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_Interests) {
    aryTemp = modModel.divTableCVInterests.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVInterests.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_Other) {
    aryTemp = modModel.divTableCVOther.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVOther.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_PersonalInfo) {
    aryTemp = modModel.divTableCVPersonalInfo.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVPersonalInfo.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_Personality) {
    aryTemp = modModel.divTableCVPersonality.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVPersonality.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_Responsibilites) {
    aryTemp = modModel.divTableCVResponsibilites.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVResponsibilitess.removeChild(elTemp);
      intNum = 0;
    }
  }

  if (strTable === modSchema.constCV_Skills) {
    aryTemp = modModel.divTableCVSkills.getElementsByClassName(
      "clhCopy" + strTable + "RowContainer"
    );

    while (intNum !== aryTemp.length) {
      elTemp = aryTemp[intNum];
      modModel.divTableCVSkills.removeChild(elTemp);
      intNum = 0;
    }
  }
}
