...
export const txtMSH_MailshotName = document.getElementById("txtMSH_MailshotName");
export const lblMSH_MailshotName = document.getElementById("lblMSH_MailshotName");
//header controls
...
export const divMailshotAddresses = document.getElementById("divMailshotAddresses");
export const btnPrintMailshot = document.getElementById("btnPrintMailshot");
export const btnPrintTestEnvelope = document.getElementById("btnPrintTestEnvelope");
...
export const lblMSH_PrintContact = document.getElementById("lblMSH_PrintContact");
export const chkMSH_PrintContact = document.getElementById("chkMSH_PrintContact");
...
export function funcPopulateCombobox() {
  ...
  */
  ...
  function funcCreateItems() {
    //Creates combobox items from db
    let elTemp;
    const trnTemp = dbJobSeekerCRM.transaction(modSchema.constSeekers_Mailshot_Header, "readonly");
  ...
  function funcPrintMailshotEnvelopes() {
    ...
    const funcPrintEnvelopes = () => {
      if (intNum === aryTemp.length) {
        ...
        elTemp = aryTemp[intNum];
        winTemp.document.getElementById("divCompany").innerText = elTemp.childNodes[2].innerText;
        winTemp.document.getElementById("divContact").innerText = elTemp.childNodes[3].innerText;
        winTemp.document.getElementById("divAddress1").innerText = elTemp.childNodes[4].innerText;
        winTemp.document.getElementById("divAddress2").innerText = elTemp.childNodes[5].innerText;
        winTemp.document.getElementById("divAddress3").innerText = elTemp.childNodes[6].innerText;
        winTemp.document.getElementById("divTownCity").innerText = elTemp.childNodes[7].innerText;
        winTemp.document.getElementById("divPostcode").innerText = elTemp.childNodes[8].innerText;
        winTemp.print();
      }
    };
...
export function funcSelectButtonClick(event) {
  ...
  */
  ...
  elTemp = divMailshotAddresses;
  aryTableRows = elTemp.getElementsByClassName("clhAddressRowContainer");

  for (intNum1 = 0; intNum1 < aryTableRows.length; intNum1++) {
    elTemp1 = aryTableRows[intNum1];

    if (elTemp1.id === "divAddressRowContainer" + intNum) {
      break;
    }
  }
  ...
  //populate text boxes
  txtMSL_CompanyName.value = elTemp1.childNodes[2].innerText;
  txtMSL_Contact.value = elTemp1.childNodes[3].innerText;
  txtMSL_Address1.value = elTemp1.childNodes[4].innerText;
  txtMSL_Address2.value = elTemp1.childNodes[5].innerText;
  txtMSL_Address3.value = elTemp1.childNodes[6].innerText;
  txtMSL_TownCity.value = elTemp1.childNodes[7].innerText;
  txtMSL_Postcode.value = elTemp1.childNodes[8].innerText;
  strTemp = elTemp1.childNodes[9].innerText;
...
*/
  ...
  const dbRequest = dbJobSeekerCRM.transaction(modSchema.constSeekers_Mailshot_Header, "readwrite");
  ...
  dbQueryHead.onsuccess = (event) => {
    ...
    if (dbCursorHead) {
      ...
      const objLines = dbRequest.objectStore(modSchema.constSeekers_Mailshot_Lines);
      ...
      dbQueryLines.onsuccess = (event) => {
        ...
        if (dbCursorLines) {
          if (dbCursorLines.value.MSH_MailshotName === txtMSH_MailshotName.value) {
...
*/
  ...
  function funcSaveLines() {
    ...
    //save lines
    dbAddLines = dbJobSeekerCRM.transaction(modSchema.constSeekers_Mailshot_Lines, "readwrite");
    ...
    objTempLines = dbAddLines.objectStore(modSchema.constSeekers_Mailshot_Lines);
  ...
  //save header
  dbAddHead = dbJobSeekerCRM.transaction(modSchema.constSeekers_Mailshot_Header, "readwrite");
...
export function funcInitSchema() {
  //get schema
  objTableHeader = modSchema.funcGetSchema(modSchema.constSeekers_Mailshot_Header);
  objTableLines = modSchema.funcGetSchema(modSchema.constSeekers_Mailshot_Lines);
}
