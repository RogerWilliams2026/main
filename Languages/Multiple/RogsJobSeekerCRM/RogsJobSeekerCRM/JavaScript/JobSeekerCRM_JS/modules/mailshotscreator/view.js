...
export function funcInitTable() {
  ...
  */
  ...
  aryTemp = modModel.divMailshotAddresses.getElementsByClassName("clhAddressRowContainer");
...
export function funcInitView() {
  ...
  */
  ...
  for (intNum = 0; intNum < aryElements.length; intNum++) {
    //get field name
    strName = aryElements[intNum].id.substring(3, aryElements[intNum].id.length);
...
export function funcRenumberRowsResetPositions() {
  ...
  */
  ...
  //get table
  elTable = modModel.divMailshotAddresses; //document.getElementById();
...
*/
  ...
  //read header records
  trnHead = modModel.dbJobSeekerCRM.transaction(modSchema.constSeekers_Mailshot_Header, "readonly");
  ...
  keyRangeHead = IDBKeyRange.only(modModel.cmbID.options[modModel.cmbID.selectedIndex].text);
  ...
  dbQueryHead.onsuccess = (event) => {
    ...
    if (dbCursorHead) {
      ...
      modModel.objTableHeader.aryFields[1].fieldValue = dbCursorHead.value.MSH_MailshotName;
      modModel.objTableHeader.aryFields[2].fieldValue = dbCursorHead.value.MSH_CVPath;
      modModel.objTableHeader.aryFields[3].fieldValue = dbCursorHead.value.MSH_LetterPath;
      modModel.objTableHeader.aryFields[4].fieldValue = dbCursorHead.value.MSH_PrintedDate;
      modModel.objTableHeader.aryFields[5].fieldValue = dbCursorHead.value.MSH_PrintContact;
      ...
      */
      ...
      dbQueryLines.onsuccess = (event) => {
        ...
        if (dbCursorLines) {
          modModel.txtMSL_CompanyName.value = dbCursorLines.value.MSL_CompanyName;
