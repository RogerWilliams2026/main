...
namespace RogStock2025_Utilities.Screens
{
    public partial class frmGroups_Utilities : Form
    {
            ...
            try
            {
                using (SQLConn = new SqlConnection(Modules.clsData_Utilities.CNST_STR_ODBC))
                {
                    ...
                    if (!blnNew)
                    {
                        ...
                        SQLCmd.CommandText = "DELETE FROM " + Modules.clsData_Utilities.CNST_STR_MENU_GROUPS + " WHERE GRP_Group = '" + this.CMBGRP_Group.Text + "';";
            ...
            */
            ...
            try
            {
                using (SQLConn = new SqlConnection(Modules.clsData_Utilities.CNST_STR_ODBC))
                {
                    ...
                    try
                    {
                        SQLCmd = new SqlCommand("DELETE FROM " + Modules.clsData_Utilities.CNST_STR_MENU_GROUPS + " WHERE GRP_Group = '" + this.CMBGRP_Group.Text + "';", SQLConn);
                        //assign commands to the transaction
                        SQLCmd.Transaction = SQLTrans;
                        //delete groups
                        SQLCmd.ExecuteNonQuery();
                        SQLCmd.CommandText = "DELETE FROM " + Modules.clsData_Utilities.CNST_STR_MENU_USERGROUPS + " WHERE USRGRP_Group = '" + this.CMBGRP_Group.Text + "';";
        ...
        private void TVMenuItems_DrawNode(object sender, DrawTreeNodeEventArgs e)
        {
