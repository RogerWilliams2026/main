                ...
                using (SQLConn = new SqlConnection(Modules.clsData_Utilities.CNST_STR_ODBC))
                {
                    ...
                    foreach (string strTemp in LSTUserGroups)
                    {
                        if (!blnNew)
                        {
                            ...
                            SQLCmd.CommandText = "DELETE" + Modules.clsData_Utilities.CNST_STR_MENU_USERGROUPS + " WHERE USRGRP_Group = '" + strTemp + "' AND USRGRP_User ='" + this.CMBUser.Text + "';";
                            SQLCmd.ExecuteNonQuery();
                        }

                        //filter for JUST menuitems as these havw a tag of 1
                        SQLCmd.CommandText = "INSERT INTO " + Modules.clsData_Utilities.CNST_STR_MENU_USERGROUPS + " (USRGRP_Group, USRGRP_User) VALUES ('" + strTemp + "','" + this.CMBUser.Text + "');";
                        SQLCmd.ExecuteNonQuery();
                    }
            ...
            */
            ...
            try
            {
                using (SQLConnMenuItems = new SqlConnection(Modules.clsData_Utilities.CNST_STR_ODBC))
                {
                    ...
                    SQLCmdMenuItems = new SqlCommand("SELECT DISTINCT GRP_Group FROM " + Modules.clsData_Utilities.CNST_STR_MENU_GROUPS + " ORDER BY GRP_Group;", SQLConnMenuItems);
                    DADMenuItems = new SqlDataAdapter(SQLCmdMenuItems);
                    DSTMenuItems = new DataSet();
                    intRows = DADMenuItems.Fill(DSTMenuItems, Modules.clsData_Utilities.CNST_STR_MENU_GROUPS);
                    ...
                    else
                    {
                        ...
                        foreach (DataRow DARTemp in DSTMenuItems.Tables[Modules.clsData_Utilities.CNST_STR_MENU_GROUPS].Rows)
                        {
            ...
            */
            ...
            try
            {
                using (SQLConnMenuItems = new SqlConnection(Modules.clsData_Utilities.CNST_STR_ODBC))
                {
                    SQLConnMenuItems.Open();
                    //load MenuItems items
                    SQLCmdMenuItems = new SqlCommand("SELECT * FROM " + Modules.clsData_Utilities.CNST_STR_MENU_USERGROUPS + " WHERE USRGRP_User ='" + this.CMBUser.Text + "';", SQLConnMenuItems);
                    DADLOTMenuItems = new SqlDataAdapter(SQLCmdMenuItems);
                    DSTLOTMenuItems = new DataSet();
                    intRows = DADLOTMenuItems.Fill(DSTLOTMenuItems, Modules.clsData_Utilities.CNST_STR_MENU_USERGROUPS);
                    ...
                    else
                    {
                        ...
                        foreach (DataRow DARTemp in DSTLOTMenuItems.Tables[Modules.clsData_Utilities.CNST_STR_MENU_USERGROUPS].Rows)
                        {
