...
namespace RogStock2026.Screens
{
    public partial class frmReportLocTRNListing : Form
    {
            ...
            */
            ...
            try
            {
                using (SQLConn = new SqlConnection(Modules.clsData.CNST_STR_ODBC))
                {
                    ...
                    SQLCmd.CommandText = "SELECT DISTINCT LOCT_ItemID FROM " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + " WHERE LOCT_ItemID IS NOT NULL ORDER BY LOCT_ItemID;";
            ...
            {
                //Whoops!
                MessageBox.Show("Error Opening Table " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + " - Check SQL Server", "Database Open Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        }
            ...
            */
            ...
            try
            {
                using (SQLConn = new SqlConnection(Modules.clsData.CNST_STR_ODBC))
                {
                    ...
                    SQLCmd.CommandText = "SELECT DISTINCT LOCT_Operation FROM " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + " WHERE LOCT_Operation IS NOT NULL ORDER BY LOCT_Operation;";
                    SQLCmd.CommandType = CommandType.Text;
                    SQLRead = SQLCmd.ExecuteReader();
        ...
        private void BTNShow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // RPTLocTRN = new ReportViewer();
                ...
                {
                    SQLCmd.CommandText = Modules.clsTables.CNST_STR_SELECT_LOCTRN + " AND " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + ".LOCT_ItemID = '" + this.CMBSTKI_ItemID.Text + "' ";
                }
                ...
                if (this.CHKAllLocations.Checked == false)
                {
                    ...
                    {
                        SQLCmd.CommandText = Modules.clsTables.CNST_STR_SELECT_LOCTRN + " AND " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + ".LOCT_Location = '" + this.CMBLOC_Location.Text + "' ";
                    }
                    else
                    {
                        //if (SQLCmd.CommandText.Contains("WHERE"))
                        //{
                            SQLCmd.CommandText += " AND " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + ".LOCT_Location = '" + this.CMBLOC_Location.Text + "' ";
                        //}
                        //else
                        //{
                ...
                if (this.CHKALLOperations.Checked == false)
                {
                    if (SQLCmd.CommandText.Length == 0)
                    {
                        SQLCmd.CommandText = Modules.clsTables.CNST_STR_SELECT_LOCTRN + "AND  " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + ".LOCT_Operation = '" + this.CMBLOCT_Operation.Text + "' ";
                    }
                    else
                    {
                        //if (SQLCmd.CommandText.Contains("WHERE"))
                        //{
                            SQLCmd.CommandText += " AND " + Modules.clsTables.CNST_STR_TABLE_LOCTRN +".LOCT_Operation = '" + this.CMBLOCT_Operation.Text + "' ";
                        //}
                        //else
                        //{
                ...
                if (this.CHKAllDates.Checked == false)
                {
                    ...
                    if (SQLCmd.CommandText.Length == 0)
                    {
                        SQLCmd.CommandText = Modules.clsTables.CNST_STR_SELECT_LOCTRN + " AND " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + ".LOCT_DateTime BETWEEN '" + strDateFrom + " 00:00:00' AND '" + strDateTo + " 23:59:59'";
                    }
                    else
                    {
                        //if (SQLCmd.CommandText.Contains("WHERE"))
                        //{
                            SQLCmd.CommandText += " AND " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + ".LOCT_DateTime BETWEEN '" + strDateFrom + " 00:00:00' AND '" + strDateTo + " 23:59:59'";
                        //}
                        //else
                        //{
                ...
                DADLocTRN.Fill(DSTLocTRN, Modules.clsTables.CNST_STR_TABLE_LOCTRN);

                if (DSTLocTRN.Tables[Modules.clsTables.CNST_STR_TABLE_LOCTRN].Rows.Count != 0)
                {
                    //main report datasource
                    RDSReportDataSource.Name = "DataSet1";
                    RDSReportDataSource.Value = DSTLocTRN.Tables[Modules.clsTables.CNST_STR_TABLE_LOCTRN];
            ...
            */
            if (Modules.clsData.CheckForStockItems())
            {
                try
                {
                    ...
                    SQLConn.Open();
                    Modules.clsView.PopulateComboBoxes(this.CMBSTKI_ItemID, Modules.clsTables.CNST_STR_TABLE_LOCTRN, "", "", "", "", "", false, false);
        ...
        private void frmReportLocTRNListing_Paint(object sender, PaintEventArgs e)
        {
            /*
