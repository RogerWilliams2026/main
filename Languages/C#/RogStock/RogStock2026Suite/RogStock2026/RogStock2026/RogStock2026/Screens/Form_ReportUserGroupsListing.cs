    ...
    public partial class frmReportUserGroupsListing : Form
    {
        ...
        private void frmReportUserGroupsListing_Load(object sender, EventArgs e)
        {
            ...
            if (Modules.clsData.CheckForLogins())
            {
                ...
                Modules.clsView.PopulateComboBoxes(this.CMBUSRGRP_User, Modules.clsTables.CNST_STR_TABLE_USERGROUPS, Modules.clsTables.GetPrimaryField(Modules.clsTables.CNST_STR_TABLE_USERGROUPS), "", "", "", "", true, false);
        ...
        private void BTNShow_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SQLConn = new SqlConnection(Modules.clsData.CNST_STR_ODBC))
                {
                    ...
                    DADUOM.Fill(DSTUOM, Modules.clsTables.CNST_STR_TABLE_USERGROUPS);
                    this.RPTUOM.LocalReport.ReportPath = Modules.clsData.CNST_STR_REPORT_USERGROUPS;

                    if (DSTUOM.Tables[Modules.clsTables.CNST_STR_TABLE_USERGROUPS].Rows.Count != 0)
                    {
                        // Must match the DataSource in the RDLC
                        RDSReportDataSource.Name = "DataSet1";
                        RDSReportDataSource.Value = DSTUOM.Tables[Modules.clsTables.CNST_STR_TABLE_USERGROUPS];
