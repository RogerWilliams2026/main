            ...
            if (Modules.clsView.ValidateRequiredFields(this))
            {
                try
                {
                    using (SQLConn = new SqlConnection(Modules.clsData.CNST_STR_ODBC))
                    {
                        ...
                        SQLCmd.CommandText = "UPDATE " + Modules.clsTables.CNST_STR_TABLE_STOCK_LOC + " SET LOC_Location = '" + this.TXTLocTo.Text + "' WHERE LOC_Location = '" + this.CMBLOC_Location.Text + "';";
                        SQLCmd.ExecuteNonQuery();

                        //create TRN loc for old location NULL for item ID as multiple items can use same location       (LOCT_ItemID,
                        SQLCmd.CommandText = "INSERT INTO " + Modules.clsTables.CNST_STR_TABLE_LOCTRN + " (LOCT_Location, LOCT_OldLocation LOCT_Qty, LOCT_Operation) " +
                        ...
                        if (blnLocLot)
                        {
                            ...
                            DSTTemp = new DataSet();
                            DADTemp.Fill(DSTTemp, Modules.clsTables.CNST_STR_TABLE_STOCK_LOT);

                            foreach (DataRow DARTemp in DSTTemp.Tables[Modules.clsTables.CNST_STR_TABLE_STOCK_LOT].Rows)
                            {
                            ...
                            //update LOT records
                            SQLCmd.CommandText = "UPDATE " + Modules.clsTables.CNST_STR_TABLE_STOCK_LOT + " SET LOT_Location = '" + this.TXTLocTo.Text + "' WHERE LOT_Location = '" + this.CMBLOC_Location.Text + "';";
                            SQLCmd.ExecuteNonQuery();
                        }
