    ...
    internal static class clsView
    {
        ...
        public static void ReadThemeData()
        {
            /*
             ...
             reads theme colours file: \Resources\rogstock2026theme.thm
            ...
            */
            ...
            try
            {
                using (SQLConn = new SqlConnection(Modules.clsData.CNST_STR_ODBC))
                {
                    ...
                    while (SQLRead.Read())
                    {
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_LOCTRN)
                        {
                            CMBTemp.Items.Add(SQLRead[1].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_LOTTRN)
                        {
                            CMBTemp.Items.Add(SQLRead[1].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_STOCK_ITEMS)
                        {
                            CMBTemp.Items.Add(SQLRead[1].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_STOCK_LOT)
                        {
                            CMBTemp.Items.Add(SQLRead[0].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_STOCK_VENDORS)
                        {
                        ...
                        //admin comboboxes
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_LOGIN)
                        {
                            CMBTemp.Items.Add(SQLRead[1].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_MENU_GROUPS)
                        {
                            CMBTemp.Items.Add(SQLRead[0].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_MENU_MENUITEMS)
                        {
                            CMBTemp.Items.Add(SQLRead[1].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_MENU_AREAS)
                        {
                            CMBTemp.Items.Add(SQLRead[1].ToString());
                        }
                        if (strTable == Modules.clsTables.CNST_STR_TABLE_MENU_USERGROUPS)
                        {
                            CMBTemp.Items.Add(SQLRead[0].ToString());
                        }
                    }
